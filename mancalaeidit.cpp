//i guess the 3rd heuristic(select option 4 is the best so far) for depth 8,then heuristic 2(select option 3)
#include <bits/stdc++.h>
using namespace std;
#define INF 999999
#define DEPTH 8
...
struct comparator
{
  ...
  {
    return (struct1.heuristicValue < struct2.heuristicValue);
  }
};
...
vector<board> getChildNodes(board parent, int heuType)
{
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    int stonesInThatBin = parent.bins[whichBin];
...

    {
      ...
      for (int j = 1;; j++) // as long as stones in that bin are not distributed
      {
        ...
        if (stonesInThatBin == 0)
        {
          ...
          {
            child.presentTurn = parent.presentTurn;
            child.earnedAdditionalMoves += 1; //
          }

          else
            child.presentTurn = 3 - parent.presentTurn; // opponent will get turn now
      ...
      // in case of min node, negate stcap - these stones were captured by the player presentTurn - guaranteed
      if (parent.presentTurn == 2)
      {
        child.stcap = (-1) * child.stcap;
        child.acquiredStones = (-1) * child.acquiredStones;
        // if parent.presentTurn was 1, more stcap is good, so +
        // if not, more stcap is good for player 2, so -
  ...
  if (currState.presentTurn == 1)
  {
    ...
    for (int i = 0; i < countChild; i++)
    {
      ...
      if (v1 > val)
        val = v1;
      if (val >= beta)
        // return beta;  //alpha>=beta hoye gele ar explore korar dorkar nai
        return val;
    }
  ...
  if (currState.presentTurn == 2)
  {
    ...
    for (int i = 0; i < countChild; i++)
    {
      ...
      if (v1 < val)
        val = v1;
      if (val <= alpha)
        // return alpha;  // //alpha>=beta hoye gele ar explore korar dorkar nai
        return val;
    }
